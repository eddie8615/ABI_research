...
failed_path = os.path.relpath('../failed2.txt')
...
output_path = '../LLD_temp/transcripts/'
...
def transcribe():
    ...
    finished = fnmatch.filter(os.listdir(output_path), '*.txt')
    done = [inst.split('.')[0] for inst in finished]

    for file in tqdm(files):
        file_name = os.path.join(audio_path, file)
        inst = file.split('.')[0]
        if inst in done:
            continue
        ...
        new_path = os.path.join(output_path, inst + '.txt')
        write_transcripts(new_path, transcript)
        print(inst, '.txt has been created')
